...
router.patch('/', requirePrivilege('public_links', 'change'), async (req, res, next) => {
  try {
    ...
    if (data.portalConfig !== undefined) {
      fields.push(`portal_config = $${i++}::jsonb`);
      values.push(JSON.stringify(data.portalConfig));
    }
    if (data.groupInfoConfig !== undefined) {
      fields.push(`group_info_config = $${i++}::jsonb`);
      values.push(JSON.stringify(data.groupInfoConfig));
    }
    if (data.calendarConfig !== undefined) {
      fields.push(`calendar_config = $${i++}::jsonb`);
      values.push(JSON.stringify(data.calendarConfig));
    }
