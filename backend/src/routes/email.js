...
router.get(
  '/standard-messages',
  requirePrivilege('email_standard_messages', 'view'),
  async (req, res, next) => {
    try {
      const rows = await tenantQuery(
        req.user.tenantSlug,
      ...
      const sendResults = await Promise.allSettled(
        recipients.map(async (member) => {
          ...
          if (giftAidDates) {
            ...
            const gaList = ga
              ? ga.items
                  .map((i) => `${fmtDate(i.date)} £${Number(i.gift_aid_amount).toFixed(2)}`)
                  .join(', ')
              : '';
          ...
          return {
            memberId: member.id,
            email: member.email,
            name: `${member.forenames} ${member.surname}`.trim(),
            msgId,
          };
        }),
      );
...
router.get('/delivery', requirePrivilege('email_delivery', 'view'), async (req, res, next) => {
  try {
    ...
    if (from) {
      params.push(from);
      sql += ` AND sent_at >= $${params.length}::timestamptz`;
    }
    if (to) {
      params.push(to);
      sql += ` AND sent_at <= $${params.length}::timestamptz`;
...
router.get(
  '/delivery/:batchId',
  requirePrivilege('email_delivery', 'view'),
  async (req, res, next) => {
    try {
      const userId = req.user.userId;
      const isAdmin = req.user.privileges?.includes('email_delivery:all');
      ...
      // Verify ownership
      const batchRows = await tenantQuery(
        req.user.tenantSlug,
        `SELECT * FROM email_batches WHERE id = $1`,
        [req.params.batchId],
      );
      if (!batchRows[0]) return res.status(404).json({ error: 'Not found' });
      if (!isAdmin && batchRows[0].user_id !== userId)
        return res.status(403).json({ error: 'Forbidden' });

      const recipients = await tenantQuery(
        req.user.tenantSlug,
        `
...
router.post(
  ...
  async (req, res, next) => {
    ...
    `,
      ...
      const refreshed = await tenantQuery(
        req.user.tenantSlug,
        `
      SELECT id, member_id, email_address, display_name, status, sendgrid_message_id, error_message, updated_at
      FROM email_recipients WHERE batch_id = $1 ORDER BY display_name
