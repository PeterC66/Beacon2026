// beacon2026/backend/src/routes/memberClasses.js
...
router.patch('/:id', requirePrivilege('member_classes', 'change'), async (req, res, next) => {
  try {
    ...
    if (data.current !== undefined) {
      fields.push(`current = $${i++}`);
      values.push(data.current);
    }
    if (data.explanation !== undefined) {
      fields.push(`explanation = $${i++}`);
      values.push(data.explanation);
    }
    if (data.isJoint !== undefined) {
      fields.push(`is_joint = $${i++}`);
      values.push(data.isJoint);
    }
    if (data.isAssociate !== undefined) {
      fields.push(`is_associate = $${i++}`);
      values.push(data.isAssociate);
    }
    if (data.showOnline !== undefined) {
      fields.push(`show_online = $${i++}`);
      values.push(data.showOnline);
    }
    if (data.fee !== undefined) {
      fields.push(`fee = $${i++}`);
      values.push(data.fee);
    }
    if (data.giftAidFee !== undefined) {
      fields.push(`gift_aid_fee = $${i++}`);
      values.push(data.giftAidFee);
    }
...
router.delete('/:id', requirePrivilege('member_classes', 'delete'), async (req, res, next) => {
  try {
    ...
    const [mc] = await tenantQuery(slug, `SELECT id, locked FROM member_classes WHERE id = $1`, [
      req.params.id,
    ]);
    if (!mc) throw AppError('Membership class not found.', 404);
    if (mc.locked) throw AppError('This membership class is locked and cannot be deleted.', 409);

    const [usage] = await tenantQuery(
      slug,
      `SELECT COUNT(*)::int AS n FROM members WHERE class_id = $1`,
      [req.params.id],
    );
    if (usage.n > 0)
      throw AppError(
...
router.get(
  '/:id/monthly-fees',
  requirePrivilege('member_classes', 'view'),
  async (req, res, next) => {
    try {
...
router.put(
  '/:id/monthly-fees',
  requirePrivilege('member_classes', 'change'),
  async (req, res, next) => {
    try {
      const slug = req.user.tenantSlug;
