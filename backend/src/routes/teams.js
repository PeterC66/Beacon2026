...
router.get('/download', requirePrivilege('groups_list', 'download'), async (req, res, next) => {
  try {
    ...
    if (format === 'excel') {
      ...
      for (const g of rows) ws.addRow(cols.map((f) => sanitizeCell(TEAM_LIST_FIELD_DEFS[f].get(g))));
...
router.get('/:id/members/download', requirePrivilege('group_records_all', 'view'), async (req, res, next) => {
  try {
    ...
    if (format === 'excel') {
      ...
      ws.getRow(1).font = { bold: true };
      for (const m of rows) ws.addRow(cols.map((f) => sanitizeCell(TEAM_MEMBER_FIELD_DEFS[f].get(m))));
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
...
// ─────────────────────────────────────────────────────────────────────────
// TEAM EVENTS  (schedule)  sub-resource:  /teams/:id/events
// ─────────────────────────────────────────────────────────────────────────
  ...
  try {
    ...
    const events = await tenantQuery(
      ...
      `SELECT ge.id, ge.event_date, ge.start_time, ge.end_time,
              ge.group_id, 'team'::text AS group_type,
