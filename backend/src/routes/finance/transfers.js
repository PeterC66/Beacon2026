...
router.get(
  '/transfers',
  requirePrivilege('finance_transfer_money', 'view'),
  async (req, res, next) => {
    try {
...
router.post(
  '/transfers',
  requirePrivilege('finance_transfer_money', 'create'),
  async (req, res, next) => {
    try {
...
router.patch(
  '/transfers/:transferId',
  requirePrivilege('finance_transfer_money', 'change'),
  async (req, res, next) => {
    try {
      const rows = await tenantQuery(
        req.user.tenantSlug,
      ...
      if (rows.length < 2) throw AppError('Transfer not found.', 404);
      if (rows.some((r) => r.cleared_at))
        throw AppError('Cleared transfers cannot be changed.', 400);
      ...
      if (data.date !== undefined) {
        outFields.push(`date = $${oi++}::date`);
        outVals.push(data.date);
      }
      if (data.amount !== undefined) {
        outFields.push(`amount = $${oi++}::numeric`);
        outVals.push(data.amount);
      }
      if (data.from_account_id !== undefined) {
        outFields.push(`account_id = $${oi++}`);
        outVals.push(data.from_account_id);
      }
      if (data.payment_ref !== undefined) {
        outFields.push(`payment_ref = $${oi++}`);
        outVals.push(data.payment_ref);
      }
      if (data.detail !== undefined) {
        outFields.push(`detail = $${oi++}`);
        outVals.push(data.detail);
      }
      if (data.remarks !== undefined) {
        outFields.push(`remarks = $${oi++}`);
        outVals.push(data.remarks);
      }
      if (data.group_id !== undefined) {
        outFields.push(`group_id = $${oi++}`);
        outVals.push(data.group_id);
      }
      if (outFields.length > 0) {
        ...
        await tenantQuery(
          req.user.tenantSlug,
          `UPDATE transactions SET ${outFields.join(', ')} WHERE id = $${oi}`,
          outVals,
        );
      }
      ...
      let ii = 1;
      if (data.date !== undefined) {
        inFields.push(`date = $${ii++}::date`);
        inVals.push(data.date);
      }
      if (data.amount !== undefined) {
        inFields.push(`amount = $${ii++}::numeric`);
        inVals.push(data.amount);
      }
      if (data.to_account_id !== undefined) {
        inFields.push(`account_id = $${ii++}`);
        inVals.push(data.to_account_id);
      }
      if (data.payment_ref !== undefined) {
        inFields.push(`payment_ref = $${ii++}`);
        inVals.push(data.payment_ref);
      }
      if (data.detail !== undefined) {
        inFields.push(`detail = $${ii++}`);
        inVals.push(data.detail);
      }
      if (data.remarks !== undefined) {
        inFields.push(`remarks = $${ii++}`);
        inVals.push(data.remarks);
      }
      if (data.group_id !== undefined) {
        inFields.push(`group_id = $${ii++}`);
        inVals.push(data.group_id);
      }
      if (inFields.length > 0) {
        ...
        await tenantQuery(
          req.user.tenantSlug,
          `UPDATE transactions SET ${inFields.join(', ')} WHERE id = $${ii}`,
          inVals,
        );
      }
...
router.delete(
  '/transfers/:transferId',
  requirePrivilege('finance_transfer_money', 'delete'),
  async (req, res, next) => {
    try {
      const rows = await tenantQuery(
        req.user.tenantSlug,
      ...
      if (rows.length < 2) throw AppError('Transfer not found.', 404);
      if (rows.some((r) => r.cleared_at))
        throw AppError('Cleared transfers cannot be deleted.', 400);
      await tenantQuery(req.user.tenantSlug, `DELETE FROM transactions WHERE transfer_id = $1`, [
        req.params.transferId,
      ]);
