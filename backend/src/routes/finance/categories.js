...
router.get(
  '/categories',
  requirePrivilege('finance_categories', 'view'),
  async (req, res, next) => {
    try {
...
router.post(
  '/categories',
  requirePrivilege('finance_categories', 'create'),
  async (req, res, next) => {
    try {
...
router.patch(
  '/categories/:id',
  requirePrivilege('finance_categories', 'change'),
  async (req, res, next) => {
    try {
...
router.delete(
  '/categories/:id',
  requirePrivilege('finance_categories', 'delete'),
  async (req, res, next) => {
    try {
      const [current] = await tenantQuery(
        req.user.tenantSlug,
      ...
      if (used)
        throw AppError(
          'Cannot delete a category that has transactions. Make it inactive instead.',
          400,
        );

      await tenantQuery(req.user.tenantSlug, `DELETE FROM finance_categories WHERE id = $1`, [
        req.params.id,
      ]);
