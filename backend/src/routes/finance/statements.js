...
async function getStatementData(tenantSlug, accountId, yearNum) {
  ...
  let accounts;
  if (isAll) {
    accounts = await tenantQuery(
      tenantSlug,
      `SELECT id, name, balance_brought_forward::float FROM finance_accounts WHERE active = true ORDER BY sort_order, name`,
    );
  } else {
    const requestedIds = accountId
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
    ...
    accounts = await tenantQuery(
      tenantSlug,
      `SELECT id, name, balance_brought_forward::float FROM finance_accounts WHERE id = ANY($1::text[]) ORDER BY sort_order, name`,
      [requestedIds],
    );
  }
  ...
  const allActive = await tenantQuery(
    tenantSlug,
    `SELECT COUNT(*)::int AS count FROM finance_accounts WHERE active = true`,
  );
  const isEffectivelyAll = isAll || accounts.length === (allActive[0]?.count ?? 0);
  const accountLabel = isEffectivelyAll ? 'All Accounts' : accounts.map((a) => a.name).join(', ');
...
router.get(
  '/statement',
  requireFeature('financialStatement'),
  requirePrivilege('finance_statement', 'view'),
  async (req, res, next) => {
    try {
...
router.get(
  '/statement/download',
  requireFeature('financialStatement'),
  requirePrivilege('finance_statement', 'download'),
  async (req, res, next) => {
    try {
      const { accountId, year } = req.query;
      if (!accountId) throw AppError('accountId is required.', 400);
      ...
      ws.addRow([`Financial Year ${d.yearNum} (${d.yearStart} to ${d.yearEnd})`]).font = {
        italic: true,
      };
      ws.addRow([]);
...
router.get(
  '/groups-statement',
  requireFeature('groupsStatement'),
  requirePrivilege('group_statement', 'view'),
  async (req, res, next) => {
    try {
...
router.get(
  '/groups-statement/download',
  requireFeature('groupsStatement'),
  requirePrivilege('group_statement', 'download'),
  async (req, res, next) => {
    try {
      const { from, to, showTransactions } = req.query;
      if (!from || !to) throw AppError('from and to dates are required.', 400);
      const groups = await getGroupsStatementData(req.user.tenantSlug, from, to);
      const entries =
        showTransactions === '1' || showTransactions === 'true'
