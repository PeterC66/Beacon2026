...
router.patch(
  '/accounts/:id',
  requirePrivilege('finance_accounts', 'change'),
  async (req, res, next) => {
    try {
      ...
      if (current.locked && !onlyBF)
        throw AppError(
          'This account is locked — only the balance brought forward can be changed.',
          400,
        );
      ...
      if (data.balance_brought_forward !== undefined) {
        fields.push(`balance_brought_forward = $${i++}::numeric`);
        values.push(data.balance_brought_forward);
      }
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      fields.push(`pending_config = $${i++}`);
      values.push(data.pending_config);
      fields.push(`pending_types  = $${i++}`);
      values.push(data.pending_config === 'by_type' ? data.pending_types : []);
      fields.push(`enable_refunds = $${i++}`);
      values.push(data.enable_refunds);
      fields.push(`updated_at = now()`);
      values.push(req.params.id);
...
router.delete(
  '/accounts/:id',
  requirePrivilege('finance_accounts', 'delete'),
  async (req, res, next) => {
    try {
      const [current] = await tenantQuery(
        req.user.tenantSlug,
      ...
      if (txnCheck)
        throw AppError(
          'Cannot delete an account that has transactions. Make it inactive instead.',
          400,
        );

      await tenantQuery(req.user.tenantSlug, `DELETE FROM finance_accounts WHERE id = $1`, [
        req.params.id,
      ]);
...
router.get(
  '/group-bf-setting',
  requirePrivilege('finance_accounts', 'view'),
  async (req, res, next) => {
    try {
      const [row] = await tenantQuery(
        req.user.tenantSlug,
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const rows = await tenantQuery(
        slug,
        `SELECT payment_method, account_id FROM payment_method_defaults`,
      );
  ...
  async (req, res, next) => {
    try {
      ...
      if (accountIds.length > 0) {
        const accs = await tenantQuery(
          slug,
          `SELECT id FROM finance_accounts WHERE id = ANY($1::text[])`,
          [accountIds],
        );
      ...
      logAudit(slug, {
        ...
        action: 'update',
        entityType: 'setting',
        entityId: 'payment_method_defaults',
        entityName: 'Payment method defaults',
      });
