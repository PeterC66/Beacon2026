...
export const auth = {
  ...
  forceChangePassword: (newPassword, question, answer) =>
    request('/auth/force-change-password', {
      method: 'POST',
      body: JSON.stringify({ newPassword, question, answer }),
    }),
  recover: (tenantSlug, forename, surname, postcode, email) =>
    request('/auth/recover', {
      method: 'POST',
      body: JSON.stringify({ tenantSlug, forename, surname, postcode, email }),
    }),
  recoverVerify: (tenantSlug, userId, answer) =>
    request('/auth/recover/verify', {
      method: 'POST',
      body: JSON.stringify({ tenantSlug, userId, answer }),
    }),
  updateQA: (question, answer) =>
    request('/auth/qa', { method: 'PATCH', body: JSON.stringify({ question, answer }) }),
  getQA: () => request('/auth/qa'),
};
...
export const users = {
  ...
  removeRole: (id, roleId) => request(`/users/${id}/roles/${roleId}`, { method: 'DELETE' }),
  setTempPassword: (id) => request(`/users/${id}/set-temp-password`, { method: 'POST' }),
...
export const memberClasses = {
  ...
  update: (id, data) =>
    request(`/member-classes/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  delete: (id) => request(`/member-classes/${id}`, { method: 'DELETE' }),
  getMonthlyFees: (id) => request(`/member-classes/${id}/monthly-fees`),
  saveMonthlyFees: (id, data) =>
    request(`/member-classes/${id}/monthly-fees`, { method: 'PUT', body: JSON.stringify(data) }),
};
...
export const memberStatuses = {
  ...
  update: (id, data) =>
    request(`/member-statuses/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
...
export const members = {
  ...
  lapse: (memberIds) =>
    request('/members/lapse', { method: 'POST', body: JSON.stringify({ memberIds }) }),
  ...
  create: (data, confirmed) =>
    request(`/members${confirmed ? '?confirmed=1' : ''}`, {
      method: 'POST',
...
export const faculties = {
  ...
  update: (id, data) =>
    request(`/faculties/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
...
export const groups = {
  ...
  listMembers: (id, params = {}) => {
    const qs = new URLSearchParams();
    if (params.showWaiting !== undefined)
      qs.set('showWaiting', params.showWaiting ? 'true' : 'false');
    const query = qs.toString();
    return request(`/groups/${id}/members${query ? '?' + query : ''}`);
  },
  addMember: (id, data) =>
    request(`/groups/${id}/members`, { method: 'POST', body: JSON.stringify(data) }),
  updateMember: (id, memberId, data) =>
    request(`/groups/${id}/members/${memberId}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  bulkRemoveMembers: (id, memberIds) =>
    request(`/groups/${id}/members/bulk`, {
      method: 'DELETE',
      body: JSON.stringify({ memberIds }),
    }),
  bulkAddToGroup: (id, targetGroupId, memberIds) =>
    request(`/groups/${id}/members/bulk-add`, {
      method: 'POST',
      body: JSON.stringify({ memberIds, targetGroupId }),
    }),
  bulkAddToEntity: (id, targetEntityId, memberIds) =>
    request(`/groups/${id}/members/bulk-add`, {
      method: 'POST',
      body: JSON.stringify({ memberIds, targetGroupId: targetEntityId }),
    }),

  listEvents: (id) => request(`/groups/${id}/events`),
  createEvents: (id, data) =>
    request(`/groups/${id}/events`, { method: 'POST', body: JSON.stringify(data) }),
  updateEvent: (id, evId, data) =>
    request(`/groups/${id}/events/${evId}`, { method: 'PATCH', body: JSON.stringify(data) }),
  deleteEvents: (id, ids) =>
    request(`/groups/${id}/events`, { method: 'DELETE', body: JSON.stringify({ ids }) }),
  ...
  createLedgerEntry: (id, data) =>
    request(`/groups/${id}/ledger`, { method: 'POST', body: JSON.stringify(data) }),
  updateLedgerEntry: (id, entryId, data) =>
...
export const teams = {
  ...
  addMember: (id, data) =>
    request(`/teams/${id}/members`, { method: 'POST', body: JSON.stringify(data) }),
  updateMember: (id, memberId, data) =>
    request(`/teams/${id}/members/${memberId}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  bulkAddToTeam: (id, targetTeamId, memberIds) =>
    request(`/teams/${id}/members/bulk-add`, {
      method: 'POST',
      body: JSON.stringify({ memberIds, targetTeamId }),
    }),
  bulkAddToEntity: (id, targetEntityId, memberIds) =>
    request(`/teams/${id}/members/bulk-add`, {
      method: 'POST',
      body: JSON.stringify({ memberIds, targetTeamId: targetEntityId }),
    }),
  ...
  createLedgerEntry: (id, data) =>
    request(`/teams/${id}/ledger`, { method: 'POST', body: JSON.stringify(data) }),
  updateLedgerEntry: (id, entryId, data) =>
    request(`/teams/${id}/ledger/${entryId}`, { method: 'PATCH', body: JSON.stringify(data) }),
  deleteLedgerEntry: (id, entryId) =>
    request(`/teams/${id}/ledger/${entryId}`, { method: 'DELETE' }),

  listEvents: (id) => request(`/teams/${id}/events`),
  createEvents: (id, data) =>
    request(`/teams/${id}/events`, { method: 'POST', body: JSON.stringify(data) }),
  updateEvent: (id, evId, data) =>
...
export const finance = {
  ...
  configureAccount: (id, data) =>
    request(`/finance/accounts/${id}/config`, { method: 'PATCH', body: JSON.stringify(data) }),
  deleteAccount: (id) => request(`/finance/accounts/${id}`, { method: 'DELETE' }),
  getGroupBfSetting: () => request('/finance/group-bf-setting'),
  setGroupBfSetting: (enabled) =>
    request('/finance/group-bf-setting', {
      method: 'PATCH',
      body: JSON.stringify({ groupBfEnabled: enabled }),
    }),
  getPaymentMethodDefaults: () => request('/finance/payment-method-defaults'),
  setPaymentMethodDefaults: (data) =>
    request('/finance/payment-method-defaults', { method: 'PUT', body: JSON.stringify(data) }),

  listCategories: () => request('/finance/categories'),
  createCategory: (data) =>
    request('/finance/categories', { method: 'POST', body: JSON.stringify(data) }),
  updateCategory: (id, data) =>
    request(`/finance/categories/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  updateTransaction: (id, data) =>
    request(`/finance/transactions/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  deleteTransaction: (id) => request(`/finance/transactions/${id}`, { method: 'DELETE' }),
  refundTransaction: (id, data) =>
    request(`/finance/transactions/${id}/refund`, { method: 'POST', body: JSON.stringify(data) }),
  bulkPending: (ids, pending) =>
    request('/finance/transactions/bulk-pending', {
      method: 'PATCH',
  ...
  createTransfer: (data) =>
    request('/finance/transfers', { method: 'POST', body: JSON.stringify(data) }),
  updateTransfer: (id, data) =>
    request(`/finance/transfers/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  deleteTransfer: (id) => request(`/finance/transfers/${id}`, { method: 'DELETE' }),

  // Reconcile
  getReconcileData: (accountId) =>
    request(`/finance/reconcile?accountId=${encodeURIComponent(accountId)}`),
  reconcile: (data) =>
    request('/finance/reconcile', { method: 'POST', body: JSON.stringify(data) }),
  ...
  getUnbatched: (accountId) =>
    request(`/finance/batches/unbatched?accountId=${encodeURIComponent(accountId)}`),
  createBatch: (data) =>
    request('/finance/batches', { method: 'POST', body: JSON.stringify(data) }),
  addToBatch: (id, transactionIds) =>
    request(`/finance/batches/${id}/transactions`, {
      method: 'POST',
  ...
  updateBatch: (id, data) =>
    request(`/finance/batches/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
...
export const giftAid = {
  list: (params = {}) => {
    const qs = new URLSearchParams();
    if (params.year) qs.set('year', String(params.year));
    if (params.excludeClaimed !== undefined)
      qs.set('excludeClaimed', params.excludeClaimed ? '1' : '0');
    const q = qs.toString();
...
export const polls = {
  ...
  addMembers: (id, memberIds) =>
    request(`/polls/${id}/members`, { method: 'POST', body: JSON.stringify({ memberIds }) }),
  setForMember: (memberId, pollIds) =>
    request(`/polls/by-member/${memberId}`, { method: 'PUT', body: JSON.stringify({ pollIds }) }),
};
...
export const settings = {
  ...
  updateFeatureConfig: (data) =>
    request('/settings/feature-config', { method: 'PATCH', body: JSON.stringify(data) }),
};
...
export const audit = {
  ...
  deleteBefore: (before) =>
    request('/audit', { method: 'DELETE', body: JSON.stringify({ before }) }),
};
...
export const systemMessages = {
  ...
  update: (id, data) =>
    request(`/system-messages/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
};
...
export const calendar = {
  ...
  createOpenEvents: (data) =>
    request('/calendar/open-events', { method: 'POST', body: JSON.stringify(data) }),
  updateOpenEvent: (id, data) =>
    request(`/calendar/open-events/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  deleteOpenEvents: (ids) =>
    request('/calendar/open-events', { method: 'DELETE', body: JSON.stringify({ ids }) }),

  // Single event
  getEvent: (id) => request(`/calendar/events/${id}`),
  searchEvents: (q, limit = 20) =>
    request(`/calendar/events/search?q=${encodeURIComponent(q)}&limit=${limit}`),

  // Event members
  listEventMembers: (eventId) => request(`/calendar/events/${eventId}/members`),
  addEventMembers: (eventId, memberIds, isOrganiser = false) =>
    request(`/calendar/events/${eventId}/members`, {
      method: 'POST',
      body: JSON.stringify({ memberIds, isOrganiser }),
    }),
  copyGroupToEvent: (eventId) =>
    request(`/calendar/events/${eventId}/members/from-group`, { method: 'POST' }),
  updateEventMember: (eventId, memberId, data) =>
    request(`/calendar/events/${eventId}/members/${memberId}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
...
export const eventTypes = {
  ...
  update: (id, data) =>
    request(`/event-types/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
...
export const reports = {
  ...
  runSql: (sql) => request('/reports/sql/run', { method: 'POST', body: JSON.stringify({ sql }) }),
  downloadSql: (sql) =>
...
export const membershipCards = {
  ...
  markPrinted: (memberIds) =>
    request('/membership-cards/mark-printed', {
      method: 'POST',
      body: JSON.stringify({ memberIds }),
    }),
};
...
export const email = {
  ...
  saveStandardMessage: (data) =>
    request('/email/standard-messages', { method: 'POST', body: JSON.stringify(data) }),
  ...
  unblock: (emailAddr) =>
    request('/email/unblocker', { method: 'POST', body: JSON.stringify({ email: emailAddr }) }),
};

export const letters = {
  listStandardLetters: () => request('/letters/standard-letters'),
  saveStandardLetter: (data) =>
    request('/letters/standard-letters', { method: 'POST', body: JSON.stringify(data) }),
