...
function portalRequest(slug, path, options = {}) {
  ...
  return fetch(`${BASE}/public/${slug}/portal/app${path}`, { ...options, headers }).then((r) =>
    r.json().then((b) => {
...
function portalBlobRequest(slug, path) {
  ...
  return fetch(`${BASE}/public/${slug}/portal/app${path}`, { headers }).then(async (r) => {
    if (!r.ok) {
...
export const portalApi = {
  ...
  leaveGroup: (slug, groupId) =>
    portalRequest(slug, `/groups/${groupId}/leave`, { method: 'POST' }),
  ...
  updatePersonalDetails: (slug, data) =>
    portalRequest(slug, '/personal-details', {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
  changePassword: (slug, data) =>
    portalRequest(slug, '/change-password', {
      method: 'POST',
      body: JSON.stringify(data),
    }),

  // Photo (10.2.4)
  uploadPhoto: (slug, data, mimeType) =>
    portalRequest(slug, '/photo', {
      method: 'POST',
  ...
  getPhotoBlob: (slug) => {
    const headers = { ...(portalToken && { Authorization: `Bearer ${portalToken}` }) };
    return fetch(`${BASE}/public/${slug}/portal/app/photo`, { headers }).then(async (r) => {
      if (!r.ok) return null;
      return r.blob();
    });
  },
  ...
  submitRenewal: (slug, data) =>
    portalRequest(slug, '/renew', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  confirmRenewal: (slug, data) =>
    portalRequest(slug, '/renewal-confirm', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
