...
export default function EventMembers({ eventId, groupId }) {
  ...
  async function handleCopyFromGroup() {
    if (
      !window.confirm(
        "Copy all current group members into this event? (Existing event members won't be duplicated.)",
      )
    )
      return;
    try {
      ...
      setError(null);
      if (result.added === 0)
        setError('No new members to copy — all group members are already registered.');
    } catch (err) {
      setError(err.message);
    }
  }
  ...
  async function handleToggleOrganiser(memberId, current) {
    try {
      ...
      setRows((prev) =>
        prev.map((r) => (r.member_id === memberId ? { ...r, is_organiser: !current } : r)),
      );
    } catch (err) {
      setError(err.message);
    }
  ...
  const btnCls =
    'bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white rounded px-4 py-2 text-sm font-medium';
      ...
      <div className="flex flex-wrap gap-2 items-center text-sm">
        ...
        {rows.length > 0 && (
          <>
            ...
            <button
              onClick={() => setSelected(new Set(rows.map((r) => r.member_id)))}
        ...
        {canChange && selected.size > 0 && (
          <button onClick={handleRemoveSelected} className="text-red-600 hover:underline ml-2">
            Remove selected
          </button>
        )}
        {canDownload && rows.length > 0 && (
          <button onClick={handleDownload} className="text-blue-700 hover:underline ml-auto">
            Download PDF
          </button>
        )}
      </div>
          ...
          <table className="min-w-full bg-white">
            ...
            <tbody className="divide-y divide-slate-200">
              {sorted.map((r) => (
                <tr
                  key={r.member_id}
                  className={selected.has(r.member_id) ? 'bg-blue-50' : 'hover:bg-slate-50'}
                >
                  {canChange && (
                  ...
                  <td className={tdCls}>
                    <Link to={`/members/${r.member_id}`} className="text-blue-700 hover:underline">
                      {r.surname}
                    </Link>
                  </td>
