...
export default function PollList() {
  ...
  function validate() {
    ...
    if (form.memberCanSet && !form.description.trim())
      errs.description = 'Description is required when members can set this poll.';
    return errs;
  }
    ...
    try {
      if (editId === 'new') {
        const created = await pollsApi.create({
          ...
          memberCanSet: form.memberCanSet,
        });
        setList((prev) =>
          [...prev, { ...created, member_count: 0 }].sort((a, b) => a.name.localeCompare(b.name)),
        );
      } else {
        const updated = await pollsApi.update(editId, {
          name: form.name.trim(),
          description: form.description.trim(),
          memberCanSet: form.memberCanSet,
        });
  ...
  async function handleDelete(poll) {
    if (!confirm(`Delete poll "${poll.name}"? This will also remove all member assignments.`))
      return;
    try {
  ...
  async function handleClearAll(poll) {
    if (
      !confirm(
        `Remove all ${poll.member_count} member assignment${poll.member_count !== 1 ? 's' : ''} from "${poll.name}"?`,
      )
    )
      return;
    try {
  ...
  return (
    <div className="min-h-screen pb-10">
      ...
      <div className="max-w-5xl mx-auto px-4 py-5">
        ...
        {error && (
          <p className="rounded-md bg-red-50 border border-red-300 px-4 py-3 text-red-700 text-sm font-medium text-center mb-4">
            {error}
          </p>
        )}
                  ...
                  {list.map((poll, i) =>
                    editId === poll.id ? (
                      ...
                      >
                        ...
                        <td
                          className="px-3 py-2 text-slate-600 max-w-[300px] truncate"
