...
const btnPrimary =
  'bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white rounded px-4 py-1.5 text-sm font-medium transition-colors';
const btnDanger =
...
export default function FinanceAccounts() {
  ...
  const navLinks = [
    { label: 'Home', to: '/' },
    { label: 'Finance categories', to: '/finance/categories' },
  ];
      ...
      <div className="max-w-2xl mx-auto px-4 py-5">
        ...
        {!loading && !error && (
          <p className="text-right mb-3">
            <Link
              to="/finance/payment-method-defaults"
                ...
                {accounts.map((acc, i) => (
                  ...
                  >
                    <td className="px-4 py-2.5">
                      <span className={acc.active ? '' : 'text-slate-400 line-through'}>
                        {acc.name}
                      </span>
                    </td>
                        ...
                        editBF[acc.id] !== undefined ? (
                          <span className="flex items-center justify-end gap-1">
                            <input
                              ...
                              value={editBF[acc.id]}
                              onChange={(e) =>
                                setEditBF((prev) => ({ ...prev, [acc.id]: e.target.value }))
                              }
                            ...
                            />
                            <button
                              onClick={() => handleSaveBF(acc)}
                              disabled={savingBF === acc.id}
                          ...
                          <button
                            onClick={() =>
                              setEditBF((p) => ({
                                ...p,
                                [acc.id]: String(acc.balance_brought_forward ?? 0),
                              }))
                            }
                            className="font-mono text-sm text-slate-700 hover:underline"
                          >
                            {Number(acc.balance_brought_forward ?? 0).toLocaleString('en-GB', {
                    ...
                    <td className="px-4 py-2.5">
                      {canChange && (
                        <Link
                          to={`/finance/accounts/${acc.id}/configure`}
