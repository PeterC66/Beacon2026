    ...
    <div className="min-h-screen pb-10">
      ...
      <div className="max-w-4xl mx-auto px-4 py-5">
        ...
        <p className="text-sm text-slate-500 text-center mb-5">
          Summary of every group's accounts from the Group Ledgers. These figures are independent
          from the main Finance Ledger.
        </p>
        ...
        <form onSubmit={handleView} className="bg-white/90 rounded-lg shadow-sm p-4 mb-5">
          ...
          <label className="flex items-center gap-2 text-sm text-slate-700 mb-3 cursor-pointer">
            <input
              type="checkbox"
              checked={showTxns}
              onChange={(e) => setShowTxns(e.target.checked)}
        ...
        {data && (
          <>
            <p className="text-sm text-slate-500 mb-3">
              Showing {groups.length} group{groups.length !== 1 ? 's' : ''} — {fmtDate(from)} to{' '}
              {fmtDate(to)}
            </p>
                  ...
                  <tbody>
                    ...
                    {groups.map((g, i) => (
                      <>
                        ...
                        >
                          <td className="px-4 py-2" title={g.short_name ? g.name : undefined}>
                            {g.short_name || g.name}
                          </td>
                          ...
                          <td className="px-4 py-2 text-right font-mono text-green-700">
                            {g.total_in > 0 ? fmtAmt(g.total_in) : ''}
                          </td>
                          <td className="px-4 py-2 text-right font-mono text-red-700">
                            {g.total_out > 0 ? fmtAmt(g.total_out) : ''}
                          </td>
                          <td
                            className={`px-4 py-2 text-right font-mono font-semibold ${g.balance >= 0 ? 'text-slate-700' : 'text-red-700'}`}
                          >
                            {fmtAmt(g.balance)}
                          </td>
                        ...
                        {showTxns &&
                          (entriesByGroup[g.id] ?? []).map((e) => (
                            <tr
                              key={e.entry_date + e.payee + e.detail}
                            ...
                            >
                              ...
                              <td></td>
                              <td className="px-4 py-1 text-right font-mono text-green-600">
                                {e.money_in > 0 ? fmtAmt(e.money_in) : ''}
                              </td>
                              <td className="px-4 py-1 text-right font-mono text-red-600">
                                {e.money_out > 0 ? fmtAmt(e.money_out) : ''}
                              </td>
                              <td></td>
                              <td></td>
                            </tr>
                    ...
                    {groups.length > 0 && (
                      <tr className="bg-slate-100 border-t border-slate-300 font-bold">
                        ...
                        <td
                          className={`px-4 py-2.5 text-right font-mono ${totalBf + totalIn - totalOut >= 0 ? 'text-slate-700' : 'text-red-700'}`}
                        >
