...
export default function FinancialStatement() {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      await requestBlob(
        `/finance/statement/download?accountId=${encodeURIComponent(accountId)}&year=${year}&format=xlsx`,
      );
    } catch (err) {
      alert(err.message);
    } finally {
      ...
      <div className="max-w-3xl mx-auto px-4 py-5">
            ...
            <div>
              ...
              <div className="border border-slate-300 rounded bg-white max-h-48 overflow-y-auto px-2 py-1.5">
                ...
                {accounts.map((a) => (
                  <label
                    key={a.id}
                    className="flex items-center gap-2 py-0.5 text-sm text-slate-700 cursor-pointer"
                  >
                    <input
                      type="checkbox"
        ...
        {data && data.pendingCount > 0 && (
          <div className="bg-amber-50 border border-amber-300 rounded-lg px-4 py-3 text-amber-800 text-sm mb-4">
            <strong>Note:</strong> {data.pendingCount} pending transaction
            {data.pendingCount === 1 ? '' : 's'} {data.pendingCount === 1 ? 'has' : 'have'} been
            excluded from this statement. Pending transactions are payments promised but not yet
            received.
          </div>
        )}
          ...
          <>
            ...
            <div className="text-center mb-4">
              ...
              <p className="text-sm text-slate-500">
                Financial Year {data.yearNum} ({data.yearStart} to {data.yearEnd})
              </p>
            </div>
            ...
            <div className="bg-white/90 rounded-lg shadow-sm overflow-hidden mb-4">
              ...
              <table className="w-full text-sm">
                <tbody>
                  {receipts.length === 0 && (
                    <tr>
                      <td colSpan={2} className="px-4 py-2 text-slate-400 italic text-xs">
                        No categorised receipts.
                      </td>
                    </tr>
                  )}
                  {receipts.map((r) => (
                  ...
                  <tr className="border-t border-slate-200 bg-slate-50">
                    ...
                    <td className="px-4 py-2 text-right font-semibold font-mono text-green-700">
                      {fmtAmt(data.totalIn)}
                    </td>
                  </tr>
                </tbody>
              ...
              <table className="w-full text-sm">
                <tbody>
                  ...
                  <tr className="border-t border-slate-200 bg-slate-50">
                    ...
                    <td className="px-4 py-2 text-right font-semibold font-mono text-red-700">
                      {fmtAmt(data.totalOut)}
                    </td>
                  </tr>
                  <tr className="bg-slate-100 border-t border-slate-300">
                    <td className="px-4 py-2 font-bold">Net Surplus / (Deficit)</td>
                    <td
                      className={`px-4 py-2 text-right font-bold font-mono ${data.totalIn - data.totalOut >= 0 ? 'text-green-700' : 'text-red-700'}`}
                    >
