...
export default function RoleEditor() {
  ...
  const hasOtherActions = resources.some((r) =>
    r.actions.some((a) => !STANDARD_ACTIONS.includes(a)),
  );
  ...
  const colHeadCls =
    'text-[#0000cc] italic font-normal text-center px-1.5 py-1 border border-slate-300 text-sm cursor-pointer select-none';
  const colHeaders = (
    <>
      ...
      {STANDARD_ACTIONS.map((a) => (
        <th
          ...
          style={{ minWidth: 52 }}
          onClick={() => canEdit && toggleColumn(a)}
        >
        ...
        <div className="bg-white/90 rounded-lg shadow-sm p-4 sm:p-6 mb-6 space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              onChange={(e) => {
                markDirty();
                setIsCommittee(e.target.checked);
              }}
              disabled={!canEdit}
          ...
          {isNew && <p className="text-xs text-slate-400 italic">New record</p>}
              ...
              <table style={{ borderCollapse: 'collapse', width: '100%' }}>
                ...
                <tbody>
                  {resources.map((resource, i) => {
                    const resourceOtherActions = resource.actions.filter(
                    ...
                    return (
                      <>
                        {repeatHeader && (
                          <tr key={`hdr-${i}`} className="bg-white">
                            {colHeaders}
                          </tr>
                        )}
                        <tr
                          key={resource.id}
                          style={{ backgroundColor: i % 2 === 0 ? '#ffffcc' : '#f0f0f0' }}
                        >
                          <td
                            style={{
                              color: '#0000cc',
