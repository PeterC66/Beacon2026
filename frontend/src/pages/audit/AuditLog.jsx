...
export default function AuditLog() {
  ...
  async function handleDelete(e) {
    ...
    try {
      ...
      setDeleteMsg({
        type: 'success',
        text: `${res.deleted} entr${res.deleted === 1 ? 'y' : 'ies'} deleted.`,
      });
      load();
    } catch (err) {
    ...
    <div className="min-h-screen pb-10">
      ...
      <div className="max-w-6xl mx-auto px-4 py-5">
        ...
        <form
          ...
          className="bg-white/90 rounded-lg shadow-sm p-3 mb-4 flex flex-wrap gap-4 items-end"
        >
          <div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 whitespace-nowrap">
                            <span
                              className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${act.cls}`}
                            >
                              {act.label}
                            </span>
                          </td>
                          ...
                          <td className="px-3 py-2">
                            {canView && (
                              <button
                                type="button"
                                className="text-blue-700 hover:underline"
                                onClick={() => navigate(`${entityRoute}/${e.entity_id}`)}
                              >
                                view
                              </button>
                            )}
            ...
            {can('audit_trail', 'delete') && (
              <div className="bg-white/90 rounded-lg shadow-sm p-4 border border-slate-200">
                ...
                <form onSubmit={handleDelete} className="flex flex-wrap gap-3 items-end">
                  ...
                  {deleteMsg && (
                    <p
                      className={`text-sm self-center ${deleteMsg.type === 'success' ? 'text-green-700' : 'text-red-600'}`}
                    >
                      {deleteMsg.text}
                    </p>
                  )}
