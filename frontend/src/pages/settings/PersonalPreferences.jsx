...
function scorePassword(pw) {
  ...
  if (pw.length >= 10) {
    ...
    hints.push('At least 10 characters');
  }
  if (pw.length >= 14) {
    score++;
  }
  if (/[A-Z]/.test(pw)) {
    ...
    hints.push('Include an uppercase letter');
  }
  if (/[a-z]/.test(pw)) {
    score++;
  } else {
    hints.push('Include a lowercase letter');
  }
  if (/[0-9]/.test(pw)) {
    score++;
  } else {
      ...
      <div className="max-w-xl mx-auto px-4 py-5">
                ...
                {[
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => {
                        markDirty();
                        setPrefs((p) => ({ ...p, colorTheme: e.target.value }));
                      }}
                      className="accent-blue-600"
                    />
                    {opt.label}
          ...
          <form onSubmit={handleSavePrefs} className="space-y-4" noValidate>
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  markDirty();
                  setPrefs((p) => ({ ...p, sortBy: e.target.value }));
                }}
                className={inputCls}
              >
            ...
            <div>
              <label
                htmlFor="pref-display-format"
                className="block text-sm font-medium text-slate-700 mb-1"
              >
                Display format
              </label>
              <select
                id="pref-display-format"
                name="displayFormat"
                value={prefs.displayFormat}
                onChange={(e) => {
                  markDirty();
                  setPrefs((p) => ({ ...p, displayFormat: e.target.value }));
                }}
                className={inputCls}
              >
            ...
            <div>
              <label
                htmlFor="pref-inactivity-timeout"
                className="block text-sm font-medium text-slate-700 mb-1"
              >
                Session timeout (minutes, 5–99)
              </label>
              <input
                id="pref-inactivity-timeout"
                type="number"
                ...
                onChange={(e) => {
                  ...
                  setPrefs((p) => ({
                    ...p,
                    inactivityTimeout: parseInt(e.target.value, 10) || 20,
                  }));
                }}
                className={inputCls}
                style={{ width: '6rem' }}
              ...
              <p className="text-xs text-slate-500 mt-1">
                After this many minutes of inactivity you will be logged out automatically. Default
                is 20 minutes. Do not increase on a shared computer.
              </p>
            </div>
        ...
        <div className={sectionCls}>
          ...
          <form onSubmit={handleChangePw} className="space-y-4" noValidate>
                ...
                <div className="mt-2 space-y-1">
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <div
                        key={n}
                        className={`h-1.5 flex-1 rounded-full ${n <= pwScore ? STRENGTH_BARS[pwScore] : 'bg-slate-200'}`}
                      />
                    ))}
            ...
            {pwMsg && (
              <p
                className={`text-sm font-medium ${pwMsg.type === 'success' ? 'text-green-700' : 'text-red-600'}`}
              >
                {pwMsg.text}
              </p>
            )}
        ...
        <div className={sectionCls}>
          ...
          <p className="text-sm text-slate-500 mb-4">
            Used for account recovery if you forget your password. Choose a question and answer that
            only you would know.
          </p>
          <form onSubmit={handleSaveQA} className="space-y-4" noValidate>
            <div>
              <label
                htmlFor="pref-security-question"
                className="block text-sm font-medium text-slate-700 mb-1"
              >
                Question
              </label>
              <input
                id="pref-security-question"
                type="text"
                ...
                onChange={(e) => {
                  markDirty();
                  setQa((q) => ({ ...q, question: e.target.value }));
                }}
            ...
            <div>
              <label
                htmlFor="pref-security-answer"
                className="block text-sm font-medium text-slate-700 mb-1"
              >
                Answer
              </label>
              <input
                id="pref-security-answer"
                type="text"
                name="securityAnswer"
                value={qa.answer}
                maxLength={200}
                ...
                onChange={(e) => {
                  markDirty();
                  setQa((q) => ({ ...q, answer: e.target.value }));
                }}
            ...
            {qaMsg && (
              <p
                className={`text-sm font-medium ${qaMsg.type === 'success' ? 'text-green-700' : 'text-red-600'}`}
              >
                {qaMsg.text}
              </p>
            )}
