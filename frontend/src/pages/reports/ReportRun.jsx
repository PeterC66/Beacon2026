            ...
            <>
                    ...
                    {report.parameters.map((p) => (
                      <div key={p.name}>
                        <label className="block text-sm font-medium text-slate-700 mb-1">
                          {p.label}
                          {p.required ? <span className="text-red-600"> *</span> : null}
                        </label>
                        <ParamInput
                          param={p}
                          value={values[p.name] ?? ''}
              ...
              {isSiteAdmin && (
                <details className="mt-4 text-xs text-slate-600">
                  ...
                  <pre className="mt-2 p-3 bg-slate-50 border border-slate-200 rounded overflow-x-auto whitespace-pre-wrap">
                    {report.sql_text}
                  </pre>
                </details>
              )}
...
function ParamInput({ param, value, onChange, className }) {
  ...
  const type = param.type === 'number' ? 'number' : param.type === 'date' ? 'date' : 'text';
  return (
    <input
      type={type}
      className={className}
