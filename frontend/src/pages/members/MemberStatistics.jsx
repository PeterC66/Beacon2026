            ...
            <>
                ...
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  ...
                  <div className="flex justify-between border-b border-slate-100 py-1.5">
                    ...
                    <dd className="font-medium text-slate-800">
                      {stats.advanceRenewalsWeeks} week{stats.advanceRenewalsWeeks !== 1 ? 's' : ''}
                    </dd>
                  </div>
                  <div className="flex justify-between border-b border-slate-100 py-1.5">
                    <dt className="text-slate-600">Grace lapse period</dt>
                    <dd className="font-medium text-slate-800">
                      {stats.graceLapseWeeks} week{stats.graceLapseWeeks !== 1 ? 's' : ''}
                    </dd>
                  </div>
                  <div className="flex justify-between border-b border-slate-100 py-1.5">
                  ...
                  <table className="min-w-max w-full text-sm">
                    ...
                    <tbody>
                      {stats.classStats.map((row, i) => (
                        <tr key={row.id} className={i % 2 === 0 ? 'bg-yellow-50' : 'bg-white'}>
                          ...
                          <td className={TD + ' text-right'}>
                            {pct(row.total, stats.totalCurrent)}
                          </td>
                      ...
                      <tr className="bg-slate-100 font-semibold">
                        ...
                        <td className={TD + ' text-right'}>
                          {stats.classStats.reduce((s, r) => s + r.with_email, 0)}
                        </td>
                        <td className={TD + ' text-right'}>
                          {stats.classStats.reduce((s, r) => s + r.first_year, 0)}
                        </td>
                        <td className={TD + ' text-right'}>
                          {stats.classStats.reduce((s, r) => s + r.second_year_plus, 0)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
              ...
              <section className="bg-white/90 rounded-lg shadow-sm p-4 sm:p-6">
                ...
                <dl className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                  ...
                  <div className="flex flex-col items-center bg-slate-50 rounded-lg p-4">
                    ...
                    <dd className="text-3xl font-bold text-slate-800">
                      {Number(stats.avgGroupMembers).toFixed(1)}
                    </dd>
                  </div>
                  <div className="flex flex-col items-center bg-slate-50 rounded-lg p-4">
                    <dt className="text-slate-500 text-xs uppercase tracking-wide mb-1">
                      Current members not in any group
                    </dt>
              ...
              <section className="bg-white/90 rounded-lg shadow-sm p-4 sm:p-6 space-y-4">
                ...
                <p className="text-xs text-slate-500">
                  Showing {fmtDate(stats.renewFrom)} – {fmtDate(stats.renewTo)}. "Not renewed" =
                  members whose next renewal date falls in this range who have not renewed. "New
                  members" = members who joined in this range.
                </p>
                <div className="overflow-x-auto">
                  <table className="min-w-max w-full text-sm">
                    <thead className="bg-slate-50 text-slate-600">
                    ...
                    <tbody>
                      ...
                      <tr className="bg-slate-100 font-semibold">
                        <td className={TD}>Total</td>
                        <td className={TD + ' text-right'}>
                          {stats.renewStats.reduce((s, r) => s + r.not_renewed, 0)}
                        </td>
                        <td className={TD + ' text-right'}>
                          {stats.renewStats.reduce((s, r) => s + r.new_members, 0)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
